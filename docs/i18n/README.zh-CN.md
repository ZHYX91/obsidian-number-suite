...
Markdown 笔记显示为从左到右的标题树。文件根卡片连接所有顶层标题，无标题时也保留；它显示
文件名、文件图标和顶层标题数量，不会替代同名 H1。初始概览以可读大小显示前两层实际标题
结构。点击图谱上方文件名可从子树返回整篇文档，点击缩放百分比可恢复 100%。标题卡片左
把手显示实际生效的虚拟序号；若当前显示的是
可靠识别的实体序号，则显示该实体序号；没有可用序号时退回显示标题层级。右把手始终显示
直接子标题数量，并且只展开或折叠该分支。选中卡片后可在整篇文档与该标题子树之间切换；
搜索不会改变结构计数；拖动空白处或滚动可平移，按住 Ctrl/Cmd 滚动可围绕指针缩放，Fit
可把当前可见树适合窗口。双击标题卡片或按 Enter 会跳到精确源码行。思维导图完全只读：
不会重排标题、修改 Markdown，也不会把题注计入子标题数量。
