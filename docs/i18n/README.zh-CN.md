...
- 为 `[^id]`、`[^footnote:id]` 和 `[^endnote:id]` 显示相互独立的纯虚拟脚注与尾注编号；
  重复引用复用首次编号。
...
类型化注释显示同样不修改 Markdown。`[^id]` 和 `[^footnote:id]` 属于脚注，
`[^endnote:id]` 属于尾注。脚注和尾注在每个文件内分别从 1 开始，并按首次引用顺序编号；
重复引用复用首次编号。定义缺失、重复、冲突或渲染不匹配时保持 Obsidian 原样。采用两个空格
