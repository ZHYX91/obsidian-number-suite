...
- 使用 Number Suite 文档大纲查看 H1-H9 标题及 Figure、Table、Equation、Code 题注，点击
  条目可跳到对应源码行。
- 使用只读的二维标题思维导图查看当前文档；每个标题是一张卡片，左把手在可用时显示最终
  生效的序号，右把手显示直接子标题数量并负责展开或折叠。
...
可从命令面板执行**打开标题思维导图**，或单击“文档大纲”标题栏中的思维导图按钮，把当前
Markdown 笔记显示为有序标题树。文件根卡片连接所有顶层标题，无标题时也保留；它显示文件名、
文件图标和顶层标题数量，不会替代同名 H1。初始概览以可读大小显示前两层实际标题结构。点击
图谱上方文件名可从子树返回整篇文档，点击缩放百分比可恢复 100%。标题卡片左把手显示实际
生效的虚拟序号；若当前显示的是可靠识别的实体序号，则显示该实体序号；没有可用序号时退回
显示标题层级。右把手始终显示直接子标题数量，并且只展开或折叠该分支。选中卡片后可在整篇
文档与该标题子树之间切换，可一次显示 1、2、3 层或全部结构层级，并可在从左到右与从上到下
布局之间切换。搜索显示当前命中序号；Enter 与 Shift+Enter 可前后遍历命中项且不改变结构计数。
拖动空白处或滚动可平移，按住 Ctrl/Cmd 滚动可围绕指针缩放，触摸设备支持双指捏合缩放，
也可使用 +、-、0 或工具栏按钮执行缩放与 Fit。双击标题卡片或在卡片主体上按 Enter 会跳到
精确源码行。思维导图完全只读：不会重排标题、修改 Markdown，也不会把题注计入子标题数量。
...
当前笔记的数字输入会在短暂停顿、按 Enter 或离开输入框后保存。
保存失败时保留输入，并提供重试；大纲导航会留在来源窗格。
