...
`NumberSuiteSidebarView` 是右侧一个常驻 `ItemView`，内部有两个选项卡。文档大纲与标题
思维导图共用 Markdown 来源绑定适配器：仍有效的原始同文件窗格优先，其次是当前活动同文件
窗格，再其次是唯一的同文件窗格；没有已绑定来源而多个窗格存在歧义时退回 Vault 缓存读取。
大纲渲染纯逻辑投影，并用源码身份而不是裸行号调和标题折叠状态；当前笔记选项卡持有可复用
...
它沿用同一窗格绑定与精确源码行导航，可按从左到右或从上到下布局渲染 SVG 结构边。纯逻辑
辅助层负责 1/2/3/全部结构层级展开和围绕锚点的视口缩放计算。卡片左把手显示标题序号或 Hn
回退，右把手始终显示直接子标题总数，并只改变当前会话的分支折叠状态。视图支持整篇文档/
所选标题子树、搜索遍历、空白处平移、Ctrl/Cmd+滚轮或双指缩放、键盘缩放/Fit 和工具栏 Fit；
...
内部 padding，源码与对象范围保持不变。composition 状态不会全局关闭语义显示：选择过滤只
恢复当前题注或引用，其他胶囊保持可见；未绑定题注保留行内组件。引用装饰把完整视觉源码替换为可聚焦胶囊，并携带精确
...
重试状态。最近批处理快照单独保存在 `recovery.json`；设置重置不得删除它。插件启动只创建
延迟恢复会话，不读取恢复文件；第一次进入批处理提交边界或请求批处理撤销时才以 single-flight
方式加载，并在读取或替换任何恢复状态之前完成该加载。自定义方案修改或删除前的模板进入清理
历史，直至用户明确清除。

Markdown 扫描器共享块级保护和保留源码偏移的行内注释范围；题注原文与用于检测引用及块 ID
的遮罩文本分开。按文件保存的协调器让已提交但保存失败的草稿跨面板生命周期保留。语义
三方合并保留独立修改，同字段的不同修改必须由用户明确选择草稿后才能覆盖。结构无效的
临时恢复数据回退到正式快照，不删除任何候选文件。
